...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('image-cropper.cropImage', async (uri) => {
        if (!uri) {
    ...
    function openCropper(uri) {
        const panel = vscode.window.createWebviewPanel('imageCropper', 'Image Cropper', vscode.ViewColumn.One, {
            enableScripts: true,
...
async function saveImage(originalPath, data, saveAs) {
    try {
        ...
        vscode.window.showInformationMessage(`Image saved successfully! Size: ${fileSize} KB`);
    }
    catch (error) {
        vscode.window.showErrorMessage(`Failed to save image: ${error}`);
    }
}
function getWebviewContent(imageUri, imageName) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
